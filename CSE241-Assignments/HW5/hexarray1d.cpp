...
namespace Mehmet{
	...
	HexArray1D::HexArray1D(int board_size_val) : AbstractHex(board_size_val) {
		set_board();
		print();
		set_user_select(1);
		finish_control();
	...
	HexArray1D::HexArray1D(int board_size_val,int game_type_val) : AbstractHex(board_size_val,game_type_val) {
		set_board();
		print();
		set_user_select(1);
		finish_control();
	...
	HexArray1D::HexArray1D(const HexArray1D& game): AbstractHex(game){
       
       int i,j;
       cell_states user;
       int destinated_cell_val;
       int index;
       ...
       for(i=0;i<get_board_size();i++){
        	for(j=0;j<get_board_size();j++){
        		index=one_dim(i,j);
        		user=game.hexCells[index].get_user_type();
        		destinated_cell_val=game.hexCells[index].get_destinated_cell();
        		hexCells[index].set_user_type(user);
        		hexCells[index].set2_destinated_cell(destinated_cell_val);
        	}
        }
	...
	HexArray1D& HexArray1D::operator =(const HexArray1D&game){
       ...
       int destinated_cell_val;
       int index;
       int old_board_size=get_board_size();

       AbstractHex::operator =(game);

       if(old_board_size!=game.get_board_size()){
       	  free(hexCells);
       	  set_board();
       }
    ...
    /* This function finds which index a cell in the two-dimensional board will correspond to on the one-dimensional board according to the pos and row parameters. */

	int HexArray1D::one_dim(int row,int pos) const{
        
		return ((get_board_size()*row)+pos);
	}


	/* /* This function sets the position,row and user type of cells in the one dimensional dynamic Cell array. */

	void HexArray1D::set_board(){

		    int i,j;
		    int index;

		    hexCells=(Cell *)calloc(get_board_size()*get_board_size(),sizeof(Cell));

		    if(hexCells!=nullptr){
		     for(i=0;i<get_board_size();i++){
		     	for(j=0;j<get_board_size();j++){
					index=one_dim(i,j);
					hexCells[index].set_pos(65+j);
		            hexCells[index].set_row(i+1);
		            hexCells[index].set_user_type(empty);
		            hexCells[index].set_destinated_cell();
			   }
		   }
		}
    ...
    void HexArray1D::set_destinated_board(){

			int i;

			for(i=0;i<get_board_size()*get_board_size();i++){
				hexCells[i].set_destinated_cell();
			}
	...
	int HexArray1D::in_board_area(int pos=0,int row=0) const{
    ...
    int HexArray1D::control_empty_cell(int pos_val,int row_val) const{

    	    int index=one_dim(row_val,pos_val);

		    if(hexCells[index].get_user_type()==empty)
		    	return 1;
		    else
		    	return 0;
    ...
    /* This function checks whether there are users in each cell of the one-dimensional dynamic Cell array. */

	int HexArray1D::all_board_fill() const{
	 ...
	 int HexArray1D::control_same_cell1(int row,int pos) const{

		  decltype(0)counter=0;
		  int index1;
		  int index2=one_dim(row,pos);

		  if(in_board_area(pos,row+1)==1){
		  	index1=one_dim(row+1,pos);
		    if(hexCells[index1].get_destinated_cell()!=1 && hexCells[index2].get_user_type()==user1 && hexCells[index1].get_user_type()==user1)
		      counter++;
		  }

		  if(in_board_area(pos-1,row)==1){
		  	index1=one_dim(row,pos-1);
		    if(hexCells[index1].get_destinated_cell()!=1 && hexCells[index2].get_user_type()==user1 && hexCells[index1].get_user_type()==user1)
		      counter++;
		  }
    ...
    int HexArray1D::control_same_cell2(int row,int pos) const{
		  ...
		  if(in_board_area(pos,row+1)==1){
		  	index1=one_dim(row+1,pos);
		    if(hexCells[index1].get_destinated_cell()!=1 && hexCells[index2].get_user_type()==user2 && hexCells[index1].get_user_type()==user2)
		      counter++;
		  }
		...
		int HexArray1D::control_empty_neighbors(int col,int row) const{
        ...
        int HexArray1D::user_computer(){
		 ...
		 int HexArray1D::get_command(int &pos,int &row){
          ...
          void HexArray1D::finish_control(){

			int board_full=all_board_fill();
			int winner=get_winner_user();
        ...
        /* This function prints the one-dimensional dynamic cell array to the screen. */
			...
			for(i=0;i<get_board_size();i++) {
		     ...
		     for(j=0;j<get_board_size();j++){
		     	 index=one_dim(i,j);
		     		if(hexCells[index].get_user_type()==empty)
		     		   cout<<". ";
			        ...
			        else if(hexCells[index].get_user_type()==user1){
			               if(hexCells[index].get_destinated_cell()==0)
			                cout<<"x ";
			                else
			                cout<<"X ";
			            }
			         else if(hexCells[index].get_user_type()==user2){
			            	if(hexCells[index].get_destinated_cell()==0)
			                cout<<"o ";
			                else
			                cout<<"O ";
			         	...
			         	cout<<hexCells[index].get_invalid_char();
		...
		int HexArray1D::readFromFile(string filename){
	    	...
	    	while(inputStream>>user){
                index=one_dim(row,col);
	    		if(user=='x')
	               hexCells[index].set_user_type(user1);
	               
	    		else if(user=='o')
	                hexCells[index].set_user_type(user2);

	            else if(user=='.')
	            	 hexCells[index].set_user_type(empty);
	            else{
	            	 hexCells[index].set_user_type(invalid);
	            	 hexCells[index].set_invalid_char(user);
	            }
	    ...
	    void HexArray1D::writeToFile(string filename) const{
		    	...
		    	while(counter<(get_board_size()*get_board_size())){
                    index=one_dim(row,col);
		    		if(hexCells[index].get_user_type()==user1)
		               outputStream<<'x';
		               
		    		else if(hexCells[index].get_user_type()==user2)
		               outputStream<<'o';
		            
		            else if(hexCells[index].get_user_type()==empty)
		            	outputStream<<'.';
		...
		void HexArray1D::setSize(){
         ...
         void HexArray1D::play(char pos_val,int row_val){

         	int index=one_dim(row_val-1,pos_val-65);
		...
		int HexArray1D::play(){
         ...
         bool HexArray1D::isEnd(int row,int pos,int user_select_val) {

         	int index1=one_dim(row,pos);
         	int index2;
	        
	       if(user_select_val==1){

		    if(pos==get_board_size()-1){
		       hexCells[index1].change_destinated_cell();
		       return true;
		   }
		    ...
		    if(in_board_area(pos,row+1)==1){
		    	index2=one_dim(row+1,pos);
		         if(hexCells[index1].get_user_type()==user1 && hexCells[index2].get_user_type()==user1 && hexCells[index2].get_destinated_cell()==0){
		            hexCells[index1].change_destinated_cell();
		    ...
		    if(in_board_area(pos,row+1)==1){
		    	index2=one_dim(row+1,pos);
		         if(hexCells[index1].get_user_type()==user2 && hexCells[index2].get_user_type()==user2 && hexCells[index2].get_destinated_cell()==0){
		            hexCells[index1].change_destinated_cell();
		            return isEnd(row+1,pos,user_select_val);
		        }
		    }
        ...
        AbstractHex::Cell HexArray1D::operator () (int row,int pos) throw(Exc1){

             try{
             	if(row<0 || row>=get_board_size() || pos<0 || pos>=get_board_size())
             		throw Exc1();

                    int index=one_dim(row,pos);
        ...
        bool HexArray1D::operator ==(HexArray1D& game){
        ...
        AbstractHex::Cell HexArray1D::last_move() throw(Exc2){
        ...
        bool HexArray1D::valid_moves() const{
        	
        	int i;

        		for(i=0;i<get_board_size()*get_board_size();i++){
        			if(hexCells[i].get_user_type()==invalid)
        				return false;
        		}
